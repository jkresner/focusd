...
var rx = {
  moment: function(s) { return new RegExp(s
    .replace('MM','([0][1-9]|[1][0-2])')
    .replace('YYYY','[1-2][0-9][0-9][0-9]')
    .replace('DD','([0][1-9]|[1-2][0-9]|[3][0-1])')) },
  esc: function(s) {
    return s == null ? null : new RegExp(s.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&'),'g')
  },
  md: {
    partial: { start: /\-\-\-\n### /g,
               ins: /\[#[A-Z]\]/g }
...
Transform.ellipt_map = function(src, map, ops) {
  ...
  for (var k=0;k<keys.length;k++) {
    ...
    if (start > -1 && end > -1 && end > start)
      r = r.slice(0, start) + ' ... ' + r.slice(end, md.lengh)
  }
  return r
}
...
Transform.abbrev = function(src, map, ops) {
  var r = src.toString()
  for (var abbr in map)
    r = r.replace(map[mapped], '<abbr>'+mapped+'</abbr>')
  return r
}
...
Transform.mark = function(input, params, ops) {
  ...
  focus.forEach(function(f) {
    ...
    f.mark.forEach(function(select) {
      ...
      var match = rx.esc(typeof select == 'string' ? select : select[idx])
...
Transform.md_partial_ins = function(md, params, ops) {
  ...
  for (var i=1;i<parts.length;i++) {
    ...
    var ins_at = new RegExp('\\[#'+pid+'\\]','g')
    var html = ops.compile.md.toHtml(lines.join('\n'))
                .replace(/\\n/g,'<br />')
                .replace(/\n/g,'')
    r = r.replace(ins_at, html)
  }       // .trim()
  return r
}
...
for (var fn in Transform) wrap(fn, Transform[fn])

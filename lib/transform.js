var rx = { escr: /[-\/\\^$*+?.()|[\]{}]/g, md: {} }
rx.md.partial = { start: /\-\-\-\n### /g, ins: /\[#[A-Z]\]/g }
rx.esc = function(s) { if ((s||{}).replace) return new RegExp(s.replace(rx.escr, '\\$&'),'g') }
...
Transform.ellipt_ln = function(r, lns, ops) {
  ...
  for (var i=0; i<lns.length; i++) {
    ...
    if (end > start) {
      l = l.slice(0, start-removed).concat(
        ['<output>\n'+(end-start > 3 ? '፧' : '⋯')+'</output>'],
        l.slice(end-removed))
      removed += (end-start)-1
    }
...
Transform.mark = function(r, p, ops) {
  console.log('=> mark '+ops.scope+'in'+ops.idx, ops.stage, p.focus)
  if (!(p.focus||{}).length > 0) return r
  var m=0;
  var idxMarks = {}
  ...
  console.log('==> in'+ops.scope+'in'+ops.idx, idxMarks)
  ...
  for (var pos in idxMarks)
    idxMarks[pos].mark.forEach(function(select) {
      ...
      //   console.log('match pos:'+pos, match);
      //   })
      // }
    })
...
var transform = {
  setOpts: function(tOps, fOps, marked) {
    var scope = tOps.id || ((fOps||{}).headerPrefix ? fOps.headerPrefix+'-' : null)
    // console.log('tsf.setOtps.scope', scope)
    ...
    else if (toHtml && tOps.id)
      toHtml.setOptions({headerPrefix: scope+'_'})

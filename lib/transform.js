...
Transform.ellipt_ln = function(r, lns, ops) {
  ...
  for (var i=0; i<lns.length; i++) {
    ...
    LOG('ellipt_ln', elt, start, end, end-start)    
    if (end > start) {
      // LOG('ellipt_ln.after', l.slice(0, start-removed).join('\n')) 
      // LOG('ellipt_ln.before', l.slice(end-removed, l.lengh).join('\n'))   
...
//       r = r.replace(match, hi('mark', '<i>'+m+'</i> '+select, f))
//     })
//     m++
//   })
//   return r

...
function Transform(src, fup, idx, opts, cb) {
  ...
  console.log('in'+idx, input.type, 'pre:', pre.join(' '), 'post:', post.join(' '), 'rendered:', rendered.join(' '))
  ...
  tns.setOpts({id,idx,stage:'pre'}, opts)
...
function fig(input, type, id, ops) {
  ops = ops || {}
  var op = setOpts(ops||{})
  var cb = function(e,r) { if (e) { console.log('e', e, 'r', r) } }
  var mup = {
    focus: [],
    in: [{
      type:type,
  ...
  tns._extend((ops||{}).transforms)
    .setOpts({idx:0,stage:'pre'}, op, marked)

  return '<figure id="'+id+'" class="in'+' '+type+'">'
     + focusd.transform(input, mup, 0, op, cb)
     + '</figure>'
}
...
function focusd(inputs, mup, ops, cb) {
  ...
  // console.log('figs', inputs.length, figs)
